...
lasso_caret <- train(
  ...
  tuneGrid = expand.grid(
    alpha = 1,
    lambda = seq(0.001, 1000, length = 100)
  )
)
...
* To make a fair comparison for LASSO, I fixed alpha = 1 in caret and tuned only lambda. Under this setting, caret selected lambda = `r lasso_caret$bestTune$lambda`, which is close to `glmnet`’s `r cv_lasso$lambda.min`.  
* Small discrepancies between the tuning parameters selected by `caret` and `glmnet` are expected. Even when alpha is fixed at 1 for LASSO, the two packages may use different lambda grids, cross-validation folds, and preprocessing procedures. These implementation differences can lead to slightly different selected lambda values, although predictive performance is typically very similar.
